...
query2a(X) :- occupation(X,patent_lawyer); occupation(X,trial_lawyer); occupation(X,civil_lawyer).
surgeon(X) :- occupation(X,oral_surgeon); occupation(X,brain_surgeon); occupation(X,heart_surgeon); occupation(X,plastic_surgeon).
california(X) :- address(X,los_angeles).
query2b(X) :-  surgeon(X), california(X).
texas(X) :- address(X,dallas); address(X,houston); address(X,college_station); address(X,san_antonio).
salaryhigh(X) :- salary(X,Salary), Salary > 100000.
query2c(X) :- salaryhigh(X), texas(X), surgeon(X).
...
canTeachMS(X,Y) :- degree(X,ms,Z), subject(Y,Z), \+ retired(X).
canTeach3(X,Y) :- canTeach2(X,Y); canTeachMS(X,Y).
...
mapcolor(WA,NT,SA,Q,NSW,V,T) :- 
    ...
    WA \== NT,WA \== SA, NT \== SA, NT \== Q, SA \== Q, SA \== NSW, Q \== NSW, NSW \== V.
